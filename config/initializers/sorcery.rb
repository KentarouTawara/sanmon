...
Rails.application.config.sorcery.configure do |config|
  ...
  #
  config.twitter.key = Rails.application.credentials.twitter.access_key_id
  config.twitter.secret = Rails.application.credentials.twitter.secret_access_key
  config.twitter.callback_url = "http://localhost:3000/oauth/callback?provider=twitter"
  config.twitter.user_info_mapping = { penname: 'screen_name' }
  #
  # config.facebook.key = ""
